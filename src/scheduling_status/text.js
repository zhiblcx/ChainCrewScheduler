...
export class SchedulingStatusController {
  ...
  async findAll(@Request() req: Request, @Body() body: string) {
    ...
    let data = await this.schedulingStatusService.findAll(user.store_id);
    ...
    if (filterData.length == 0) {
      ...
      for (let i = 0; i < restday.length; i++) {
        // 按照员工的工作时间排序
        allUser.sort((a, b) => Number(a.month_timer) - Number(b.month_timer));
        // 当天员工的状态
        const dayStatus = [];
        // 当天服务员的状态
        ...
        // 1：早班，2：中班，3：晚班
        // 所有服务员
        const all_waiter = allUser.filter((item) => item.position === '服务员');
        // 所有厨师
        const all_chef = allUser.filter((item) => item.position === '厨师');
        // 自动排班当天的所有服务员的状态
        for (let j = 0; j < rest_class.length; j++) {
          // 分配服务员
          for (let k = 0; k < rest_class[j].waiter; k++) {
            let type: string;
            if (rest_class[j].type == 1) {
              type = '早班';
            } else if (rest_class[j].type == 2) {
              type = '中班';
            } else {
              type = '晚班';
            }
            ...
            const currentStatus = {
              ...
              classes: type,
              status: '上班',
              working_hours: rest_class[j].timer,
            };
            if (
              Number(
            ...
            const currentUserIndex = allUser.findIndex(
              (item) => item.id === currentStatus.user_id,
            );
            if (currentUserIndex !== -1) {
              allUser[currentUserIndex].month_timer += Number(
                rest_class[j].timer,
              );
              allUser[currentUserIndex].week_timer += Number(
                rest_class[j].timer,
              );
            }
          }
          // 分配厨师
          for (let k = 0; k < rest_class[j].chef; k++) {
            let type: string;
            if (rest_class[j].type == 1) {
              type = '早班';
            } else if (rest_class[j].type == 2) {
            ...
            if (
              Number(
                all_chef[day_chef_status.length].week_timer +
                  rest_class[j].timer,
              ) > rule.week_more
            ) {
              break;
      ...
      for (let i = 0; i < workday.length; i++) {
        ...
        for (let j = 0; j < all_class.length; j++) {
          // 分配服务员
          for (let k = 0; k < all_class[j].waiter; k++) {
            let type: string;
            if (all_class[j].type == 1) {
              type = '早班';
            } else if (all_class[j].type == 2) {
              type = '中班';
            } else {
              type = '晚班';
            }
            ...
            const currentStatus = {
              ...
              classes: type,
              status: '上班',
              working_hours: all_class[j].timer,
            };
            if (
              Number(
                all_waiter[day_waiter_status.length].week_timer +
          ...
          // 分配厨师
          for (let k = 0; k < all_class[j].chef; k++) {
            let type: string;
            if (all_class[j].type == 1) {
              type = '早班';
            } else if (all_class[j].type == 2) {
            ...
            currentUser.month_timer += currentStatus.timer;
            const currentUserIndex = allUser.findIndex(
              (item) => item.id === currentStatus.user_id,
            );
            if (currentUserIndex !== -1) {
      ...
      data = await this.schedulingStatusService.findAll(user.store_id);
    ...
    for (let i = 0; i < allUser.length; i++) {
      ...
      if (status.length == 0) {
        ...
        if (body['week'] == 'this') {
          ...
          for (let k = 0; k < restday.length; k++) {
            for (let j = 0; j < rest_class.length; j++) {
              let type;
              if (rest_class[j].type == 1) {
                type = '早班';
              ...
              const dayStaff = await Promise.all(
                daysStaff
                  .filter((item) =>
                    dayjs(restday[k], 'YYYY/MM/DD').isSame(item.timer, 'day'),
                  )
                  .map(async (item) => {
                    const user = await this.userService.findOne(item.user_id);
                    const isValid =
                      user.position === allUser[i].position &&
                      item.classes === type;
              ...
              // 如果是服务员
              if (allUser[i].position === '服务员') {
                if (
                  people <= Number(rest_class[j].waiter) &&
                  rule.week_more >=
                    allUser[i].week_timer + Number(rest_class[j].timer)
                ) {
                  newStatus.push({
                ...
                // 如果是厨师
                if (
                  people <= Number(rest_class[j].chef) &&
                  rule.week_more >=
                    allUser[i].week_timer + Number(rest_class[j].timer)
                ) {
                  newStatus.push({
          ...
          if (remaining_time.length == 1) {
            ...
            all_class.sort((a, b) => Number(b.timer) - Number(a.timer));
            for (let k = 0; k < workday.length; k++) {
              for (let j = 0; j < all_class.length; j++) {
                let type;
                if (rest_class[j].type == 1) {
                  type = '早班';
                ...
                const dayStaff = await Promise.all(
                  daysStaff
                    .filter((item) =>
                      dayjs(workday[k], 'YYYY/MM/DD').isSame(item.timer, 'day'),
                    )
                    .map(async (item) => {
                      const user = await this.userService.findOne(item.user_id);
                      const isValid =
                ...
                const filteredDayStaff = dayStaff.filter(
                  (item) => item !== null,
                );
                const people = filteredDayStaff.length + 1;
                // 如果是服务员
                if (allUser[i].position === '服务员') {
                  if (
                    people <= all_class[j].waiter &&
                    rule.week_more >=
                      allUser[i].week_timer + Number(all_class[j].timer)
                  ) {
                    newStatus.push({
                      user_id: allUser[i].id,
                      store_id: allUser[i].store_id,
                  ...
                  // 如果是厨师
                  if (
                    people <= Number(all_class[j].chef) &&
                    rule.week_more >=
                      allUser[i].week_timer + Number(all_class[j].timer)
                  ) {
                    newStatus.push({
          ...
          if (dayStaff.length == 0) {
            ...
            for (let k = 0; k < restday.length; k++) {
              for (let j = 0; j < rest_class.length; j++) {
                ...
                const dayStaff = await Promise.all(
                  daysStaff
                    .filter((item) =>
                      dayjs(restday[i], 'YYYY/MM/DD').isSame(item.timer, 'day'),
                    )
                    .map(async (item) => {
                      const user = await this.userService.findOne(item.user_id);
                      const isValid =
            ...
            for (let k = 0; k < workday.length; k++) {
              for (let j = 0; j < all_class.length; j++) {
                ...
                if (allUser[i].position === '服务员') {
                  if (
                    people <= Number(all_class[j].waiter) &&
                    rule.week_more >=
                      allUser[i].week_timer + Number(all_class[j].timer)
                  ) {
                    newStatus.push({
